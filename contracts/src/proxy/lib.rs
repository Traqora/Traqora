...
impl ProxyStorage {
    ...
    pub fn set_multisig_proposal_count(env: &Env, count: u64) {
        env.storage()
            .instance()
            .set(&symbol_short!("ms_count"), &count);
    }
    ...
    pub fn get_storage_migration(
        ...
        from_version: u32,
        to_version: u32,
    ) -> Option<StorageMigration> {
        env.storage()
            .persistent()
            .get(&(symbol_short!("migration"), from_version, to_version))
    }
