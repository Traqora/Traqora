use soroban_sdk::{contract, contractimpl, contracttype, symbol_short, vec, Address, BytesN, Env, Symbol, Vec};
...
impl DisputeStorageKey {
    pub fn get_dispute(env: &Env, dispute_id: u64) -> Option<Dispute> {
        env.storage().persistent().get(&(symbol_short!("dispute"), dispute_id))
    }
    
    pub fn set_dispute(env: &Env, dispute_id: u64, dispute: &Dispute) {
        env.storage().persistent().set(&(symbol_short!("dispute"), dispute_id), dispute);
    }
    
    pub fn get_dispute_count(env: &Env) -> u64 {
        env.storage().instance().get(&symbol_short!("d_count")).unwrap_or(0)
    }
    
    pub fn set_dispute_count(env: &Env, count: u64) {
        env.storage().instance().set(&symbol_short!("d_count"), &count);
    }
    
    pub fn get_evidence(env: &Env, dispute_id: u64, index: u32) -> Option<Evidence> {
        env.storage().persistent().get(&(symbol_short!("evidence"), dispute_id, index))
    }
    
    pub fn set_evidence(env: &Env, dispute_id: u64, index: u32, evidence: &Evidence) {
        env.storage().persistent().set(&(symbol_short!("evidence"), dispute_id, index), evidence);
    }
    
    pub fn get_juror(env: &Env, dispute_id: u64, index: u32) -> Option<JurorSelection> {
        env.storage().persistent().get(&(symbol_short!("juror"), dispute_id, index))
    }
    
    pub fn set_juror(env: &Env, dispute_id: u64, index: u32, juror: &JurorSelection) {
        env.storage().persistent().set(&(symbol_short!("juror"), dispute_id, index), juror);
    }
    
    pub fn is_juror(env: &Env, dispute_id: u64, address: &Address) -> bool {
        env.storage().persistent().has(&(symbol_short!("is_juror"), dispute_id, address))
    }
    
    pub fn mark_as_juror(env: &Env, dispute_id: u64, address: &Address) {
        env.storage().persistent().set(&(symbol_short!("is_juror"), dispute_id, address), &true);
    }
    
    pub fn get_vote_commit(env: &Env, dispute_id: u64, juror: &Address) -> Option<VoteCommit> {
        env.storage().persistent().get(&(symbol_short!("v_commit"), dispute_id, juror))
    }
    
    pub fn set_vote_commit(env: &Env, dispute_id: u64, juror: &Address, commit: &VoteCommit) {
        env.storage().persistent().set(&(symbol_short!("v_commit"), dispute_id, juror), commit);
    }
    
    pub fn get_vote_reveal(env: &Env, dispute_id: u64, juror: &Address) -> Option<VoteReveal> {
        env.storage().persistent().get(&(symbol_short!("v_reveal"), dispute_id, juror))
    }
    
    pub fn set_vote_reveal(env: &Env, dispute_id: u64, juror: &Address, reveal: &VoteReveal) {
        env.storage().persistent().set(&(symbol_short!("v_reveal"), dispute_id, juror), reveal);
    }
    ...
    pub fn get_stake(env: &Env, dispute_id: u64, party: &Address) -> i128 {
        env.storage().persistent().get(&(symbol_short!("stake"), dispute_id, party)).unwrap_or(0)
    }
    
    pub fn set_stake(env: &Env, dispute_id: u64, party: &Address, amount: i128) {
        env.storage().persistent().set(&(symbol_short!("stake"), dispute_id, party), &amount);
    }
}
...
impl DisputeContract {
    ...
    pub fn file_dispute(
        ...
        let config = DisputeStorageKey::get_config(&env)
            .expect("Contract not initialized");
        
        let min_stake = amount * config.min_stake_percentage as i128 / 10000;
        assert!(
            passenger_stake >= min_stake,
            "Insufficient stake"
        );
        ...
        let dispute = Dispute {
            ...
            reveal_deadline: current_time + config.evidence_period + config.voting_period + config.reveal_period,
            appeal_deadline: current_time + config.evidence_period + config.voting_period + config.reveal_period + config.appeal_period,
    ...
    pub fn airline_respond(
        ...
        let mut dispute = DisputeStorageKey::get_dispute(&env, dispute_id)
            .expect("Dispute not found");
        
        assert!(dispute.airline == airline, "Not the airline in dispute");
        assert!(dispute.phase == DisputePhase::Evidence, "Evidence phase ended");
    ...
    pub fn submit_evidence(
        ...
        assert!(dispute.phase == DisputePhase::Evidence, "Not in evidence phase");
    ...
    pub fn select_as_juror(
        ...
        assert!(
            dispute.phase == DisputePhase::JurySelection || dispute.phase == DisputePhase::CommitVote,
            "Not in jury selection phase"
        );
        assert!(token_balance > 0, "Must hold TRQ tokens");
        assert!(!DisputeStorageKey::is_juror(&env, dispute_id, &juror), "Already selected");
        assert!(juror != dispute.passenger && juror != dispute.airline, "Parties cannot be jurors");
    ...
    pub fn commit_vote(
        ...
        let dispute = DisputeStorageKey::get_dispute(&env, dispute_id)
            .expect("Dispute not found");
    ...
    pub fn reveal_vote(
        ...
        let mut hash_input = vec![&env];
        hash_input.push_back(if vote_for_passenger { 1u32 } else { 0u32 });
        let salt_bytes = salt.to_array();
        for byte in salt_bytes.iter() {
            hash_input.push_back(*byte as u32);
        }
        
        let computed_hash = env.crypto().keccak256(&hash_input.to_bytes());
        assert!(
    ...
    pub fn file_appeal(
        ...
        let is_losing_party = (verdict == symbol_short!("airline") && appellant == dispute.passenger)
        ...
        dispute.reveal_deadline = new_evidence_deadline + config.voting_period + config.reveal_period;
        dispute.appeal_deadline = new_evidence_deadline + config.voting_period + config.reveal_period + config.appeal_period;
    ...
    pub fn execute_verdict(env: Env, dispute_id: u64) {
        ...
        let verdict = dispute.verdict.clone().expect("No verdict");
        assert!(verdict != symbol_short!("tie"), "Cannot execute tie verdict");
        ...
        let jury_reward_pool = total_stake_pool * config.jury_reward_pool_percentage as i128 / 10000;
