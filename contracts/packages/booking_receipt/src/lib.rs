...
crate::upgrade_timelock::UpgradeTimelock::init_upgrade_owner(&env, &owner);
    ...
    pub fn sbt_balance(env: Env, id: Address) -> i128 {
        ReceiptStorage::get_balance(&env, &id)
    }

    pub fn sbt_transfer(_env: Env, _from: Address, _to: Address, _amount: i128) {
        panic!("Non-transferable soulbound token");
    }
    ...
    pub fn sbt_burn(_env: Env, _from: Address, _amount: i128) {
        panic!("Burn not supported");
    }

    pub fn sbt_burn_from(_env: Env, _spender: Address, _from: Address, _amount: i128) {
        panic!("Burn not supported");
    }

    pub fn sbt_decimals(_env: Env) -> u32 {
        0
    }

    pub fn sbt_name(env: Env) -> String {
        ReceiptStorage::get_metadata(&env)
            .map(|m| m.name)
            .expect("Not initialized")
    }

    pub fn sbt_symbol(env: Env) -> Symbol {
        ReceiptStorage::get_metadata(&env)
            .map(|m| m.symbol)
            .expect("Not initialized")
    }
