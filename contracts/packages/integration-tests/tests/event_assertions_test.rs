...
fn test_booking_created_event() {
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one booking:created event"
    );
...
fn test_booking_paid_event() {
    ...
    let (_, _, data) = &events[0];
    let (actor, _ts, id, amt): (Address, u64, u64, i128) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
...
fn test_booking_released_event() {
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one booking:released event"
    );
...
fn test_booking_refunded_event() {
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one booking:refunded event"
    );
...
fn test_refund_requested_event() {
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one refund:requested event"
    );

    let (_, _, data) = &events[0];
    let (actor, _ts, rid, booking_id, amt): (Address, u64, u64, u64, i128) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
...
fn test_refund_approved_event() {
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one refund:approved event"
    );

    let (_, _, data) = &events[0];
    let (actor, _ts, rid, _booking_id, amt): (Address, u64, u64, u64, i128) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
...
fn test_refund_rejected_event() {
    ...
    contracts
        .refund
        .reject_refund(&actors.admin, &request_id, &Symbol::new(&env, "policy"));
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one refund:rejected event"
    );
}
...
fn test_loyalty_points_earned_event() {
    ...
    let earned = contracts
        .loyalty
        .award_points(&actors.passenger, &booking_amount, &1u64);
    ...
    let (_, _, data) = &events[0];
    let (actor, _ts, pts, booking_id): (Address, u64, i128, u64) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
    assert_eq!(
        pts, earned,
        "earned points in event should match return value"
    );
...
fn test_loyalty_points_redeemed_event() {
    ...
    contracts
        .loyalty
        .award_points(&actors.passenger, &1000_0000000i128, &1u64);
    let points_to_redeem = 500i128;
    let discount = contracts
        .loyalty
        .redeem_points(&actors.passenger, &points_to_redeem);
    ...
    assert_eq!(
        events.len(),
        1,
        "Expected exactly one points:redeemed event"
    );

    let (_, _, data) = &events[0];
    let (actor, _ts, pts, disc): (Address, u64, i128, i128) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
    assert_eq!(
        ...
        "redeemed points in event should match"
    );
    assert_eq!(
        disc, discount,
        "discount in event should match return value"
    );
}
...
fn test_loyalty_tier_upgrade_event() {
    ...
    for i in 0..5u64 {
        contracts
            .loyalty
            .award_points(&actors.passenger, &1000_0000000i128, &i);
    }
    ...
    assert!(
        events.len() >= 1,
        "Expected at least one tier:upgrade event"
    );

    let (_, _, data) = &events[0];
    let (actor, _ts, _tier): (Address, u64, Symbol) = data
        .clone()
        .try_into_val(&env)
        .expect("Event data shape mismatch");
    assert_eq!(actor, actors.passenger, "actor should be passenger");
